...
export default function CoordinatorComplaints() {
  ...
  const receivedComplaints = [
    { id: 1, student: 'Alice Johnson', subject: 'Classroom temperature too cold', category: 'Infrastructure', status: 'Open', submittedOn: '2024-02-10' },
    { id: 2, student: 'Bob Wilson', subject: 'Bullying incident', category: 'Student Behavior', status: 'In Progress', submittedOn: '2024-02-08' }
  ];

  const myComplaints = [
    { id: 3, subject: 'Need additional teaching resources', category: 'Resources', status: 'Pending', submittedOn: '2024-02-05' }
  ];
    ...
    <DashboardLayout requiredRole="coordinator">
      <div style={{
        ...
        <div style={{ display: 'flex', marginBottom: '20px' }}>
          ...
          >
            Student Complaints ({receivedComplaints.length})
          </button>
          <button
              ...
              <tbody>
                {receivedComplaints.map(complaint => (
                  <tr key={complaint.id} style={{ backgroundColor: complaint.id % 2 === 0 ? '#f8f9fa' : 'transparent' }}>
                    <td style={{ padding: 12 }}>{complaint.student}</td>
